const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/index-C6ScifuI.js","assets/index-DNCxohAG.css"])))=>i.map(i=>d[i]);
import{u as te,b as ae,r as l,j as e,A as M,_ as F,d as re}from"./index-C6ScifuI.js";import{T as se}from"./TermsModal-Blv0LBqx.js";import{g as v}from"./userUtils-BvUA9Xnv.js";const de=()=>{const m=te(),p=ae().state;if(!p)return m("/sell-gold"),null;const{amount:U,grams:$,sellRate:g,sellMode:x,preview:B,userId:k,lockedTime:P,timeLeft:G,gst:ne}=p,[f,V]=l.useState(G||300),[A,K]=l.useState(!0),[N,W]=l.useState(!1),[Y,C]=l.useState(!1),[q,E]=l.useState(!1),[n,w]=l.useState(null),[J,I]=l.useState(!1),[j,L]=l.useState(!1),[D,S]=l.useState(!1),[H,u]=l.useState(!1),[_,d]=l.useState(null),T=l.useRef(!1),b=$,h=U,R=`${M}/oxygold-api/auth/getBankDetailsByuserId`,z=`${M}/oxygold-api/digital-gold/sell/initiate`;l.useEffect(()=>{if(f<=0)return;const s=setInterval(()=>{V(a=>a<=1?(K(!1),0):a-1)},1e3);return()=>clearInterval(s)},[f]),l.useEffect(()=>{(async()=>{try{I(!0),d(null);const a=k||v();if(!a){m("/login");return}if(p!=null&&p.newBankDetails){console.log("=== USING NEW BANK DETAILS FROM STATE ==="),w(p.newBankDetails),S(!0),u(!0),setTimeout(()=>u(!1),3e3);return}console.log("=== FETCHING BANK DETAILS FROM API ==="),console.log("API URL:",`${R}?userId=${a}`);const i=await(await F(async()=>{const{default:c}=await import("./index-C6ScifuI.js").then(t=>t.ae);return{default:c}},__vite__mapDeps([0,1]))).default.getInstance().getValidAccessToken(),r=await fetch(`${R}?userId=${a}`,{method:"GET",headers:{Authorization:`Bearer ${i}`,"Content-Type":"application/json"}});if(console.log("=== BANK DETAILS API RESPONSE ==="),console.log("Response status:",r.status),console.log("Response ok:",r.ok),r.ok){const c=r.headers.get("content-type");let t;if(c&&c.includes("application/json"))t=await r.json();else{const y=await r.text();console.log("Bank details response text:",y),t=y}console.log("Bank details data:",t),console.log("Data type:",typeof t),t&&t.data&&Array.isArray(t.data)&&t.data.length>0?(console.log("Found bank details in data array:",t.data[0]),w(t.data[0]),S(!0),u(!0),setTimeout(()=>u(!1),3e3)):Array.isArray(t)&&t.length>0?(console.log("Found bank details in direct array:",t[0]),w(t[0]),S(!0),u(!0),setTimeout(()=>u(!1),3e3)):t&&typeof t=="object"&&t.accountNumber?(console.log("Found bank details as direct object:",t),w(t),S(!0),u(!0),setTimeout(()=>u(!1),3e3)):(console.log("No valid bank details found in response"),console.log("Response structure:",JSON.stringify(t,null,2)))}else console.log("Bank details API returned non-ok status:",r.status),r.status===401?d("Authentication failed. Please login again."):r.status===404?console.log("No bank details found for user"):r.status>=500?d("Server error. Please try again later."):d(`Failed to load bank details (Error ${r.status})`)}catch(a){console.error("Failed to fetch bank details:",a),a instanceof Error?a.message.includes("Failed to fetch")||a.message.includes("NetworkError")?d("Network error. Please check your connection and try again."):a.message.includes("Authentication")?d("Authentication failed. Please login again."):d("Failed to load bank details. Please try again."):d("An unexpected error occurred. Please try again.")}finally{I(!1)}})()},[k,p==null?void 0:p.newBankDetails]);const O=s=>{const a=Math.floor(s/60),o=s%60;return`${a}:${o.toString().padStart(2,"0")}`},Q=async()=>{if(A){if(!N){E(!0),setTimeout(()=>E(!1),3e3);return}if(!n){alert("Please add bank account details before proceeding.");return}if(j||T.current){console.log("=== DUPLICATE CALL BLOCKED - ALREADY PROCESSING ===");return}T.current=!0,L(!0);try{await Z()}catch(s){throw T.current=!1,L(!1),s}}},Z=async()=>{E(!1);const s=k||v();if(!s){m("/login");return}try{const a=`SELL_${Date.now()}_${Math.random().toString(36).substr(2,9)}`;if(console.log(`=== STARTING SELL CALL ${a} ===`),!h||!b||!g||!s)throw new Error("Missing required data for sell initiation");const o={userId:parseInt(s.toString()),amount:parseFloat(h.toString()),grams:parseFloat(b.toString()),purchaseType:(x==null?void 0:x.toUpperCase())==="GRAMS"?"GRAMS":"AMOUNT",pergramPrice:parseFloat(g.toString()),pergramSellingPrice:parseFloat(g.toString()),paymentMode:"BANK",productId:4};if(o.amount<=0||o.grams<=0)throw new Error("Amount and grams must be greater than 0");if(o.pergramPrice<=0||o.pergramSellingPrice<=0)throw new Error("Price per gram must be greater than 0");if(isNaN(o.amount)||isNaN(o.grams)||isNaN(o.pergramPrice)||isNaN(o.pergramSellingPrice))throw new Error("Invalid numeric values in request");console.log(`=== SELL INITIATE API CALL START ${a} ===`),console.log("Sell Initiate Request:",o),console.log("Request Body JSON:",JSON.stringify(o,null,2)),console.log("API URL:",z);const i=await re(z,{method:"POST",body:JSON.stringify(o)});if(console.log(`=== SELL INITIATE API CALL END ${a} ===`),console.log("Sell Initiate Response:",i),i&&i.success&&i.data){const{transactionId:r,beneficiaryId:c,status:t}=i.data;console.log("=== SELL INITIATE SUCCESS ==="),console.log("Transaction ID:",r),console.log("Beneficiary ID:",c),console.log("Status:",t),m("/sell-processing",{state:{transactionId:r,beneficiaryId:c,status:t,amount:h,grams:b,sellRate:g,bankDetails:n,message:i.message,userId:s,purchaseType:(x==null?void 0:x.toUpperCase())==="GRAMS"?"GRAMS":"AMOUNT",paymentMode:"BANK",productId:4}})}else throw console.error("=== SELL INITIATE FAILED ==="),console.error("Response:",i),new Error((i==null?void 0:i.message)||"Sell initiation failed - invalid response")}catch(a){console.error("=== SELL INITIATE ERROR ==="),console.error("Sell initiate error:",a),console.error("Error details:",{message:a instanceof Error?a.message:"Unknown error",stack:a instanceof Error?a.stack:void 0}),console.error("=== END ERROR LOG ==="),alert("Transaction failed: "+(a instanceof Error?a.message:"Unknown error"))}finally{console.log("=== SELL LOADING SET TO FALSE ==="),T.current=!1,L(!1)}},X=f/300*100,ee=f<=60;return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
