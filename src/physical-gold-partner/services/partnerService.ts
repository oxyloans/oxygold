...
const getPartnerAuthToken = (): string => {
    ...
    if (stored) {
        ...
        return partnerData.token || partnerData.data?.accessToken || partnerData.accessToken || "";
    }
    return "";
};
 ...
 */
const updatePartnerStoredTokens = (accessToken: string, refreshToken: string) => {
    const stored = localStorage.getItem("partner");
    if (stored) {
        const partnerData = JSON.parse(stored);
        if (partnerData.data) {
 ...
 */
const partnerAuthenticatedFetch = async (url: string, options: RequestInit = {}): Promise<Response> => {
    const headers = {
...
export const createRole = async (role: string) => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/auth/createRole`, {
        method: 'POST',
...
export const fetchDeliveryBoys = async () => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/admin/delivery/viewAllDeliveryBoys`);
    if (!response.ok) throw new Error("Failed to fetch delivery boys");
    return response.json();
};

export const deactivateDeliveryBoy = async (id: number) => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/admin/delivery/${id}/deactivate`, {
        method: 'PUT'
    });
    if (!response.ok) throw new Error("Failed to deactivate delivery boy");
    return response.json();
};
...
export const viewAllUsers = async (page: number, size: number) => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/auth/viewAllUsers?page=${page}&size=${size}`);
...
export const assignOrder = async (payload: AssignOrderPayload) => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/admin/delivery/assign`, {
        method: 'POST',
...
export const rejectOrder = async (payload: RejectOrderPayload) => {
    const response = await partnerAuthenticatedFetch(`${BASE_URL}/delivery/reject`, {
        method: 'POST',
        body: JSON.stringify(payload),
    });
    if (!response.ok) {
