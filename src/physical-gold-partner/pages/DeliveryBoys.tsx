...
import { fetchDeliveryBoys, deactivateDeliveryBoy } from '../services/partnerService';
...
const DeliveryBoys: React.FC = () => {
    ...
    const [boyToDeactivate, setBoyToDeactivate] = useState<DeliveryBoy | null>(null);
    ...
    const handleStatusToggle = (boy: DeliveryBoy) => {
        if (boy.status === 'ACTIVE') {
            setBoyToDeactivate(boy);
            setIsDeactivateModalOpen(true);
        } else {
            // "Only for deactivation don't do the activation"
            console.log("Activation not implemented yet");
        }
    };
    ...
    const confirmDeactivation = async () => {
        ...
        try {
            ...
            setDeliveryBoys(prev => prev.map(db =>
                db.id === id ? { ...db, status: 'INACTIVE' } : db
            ));
        } catch (error) {
    ...
    const columns = [
        ...
        },
        {
            header: 'Phone',
            key: 'phone',
            render: (val: string) => <span className="font-medium text-slate-600 tabular-nums">{val}</span>
        },
        {
            header: 'Status',
            key: 'status',
            render: (val: string, item: DeliveryBoy) => (
                <div className="flex items-center gap-3">
                    <Switch
                        ...
                        disabled={updatingStatus === item.id || val === 'INACTIVE'}
                    />
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${val === 'ACTIVE' ? 'text-emerald-600' : 'text-slate-400'}`}>
                        {val}
                    </span>
        ...
        <div className="space-y-6">
            <div className="flex flex-col">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold text-slate-800 tracking-tight">Delivery Boys List</h1>
                </div>
                <p className="text-[12px] text-slate-400 font-medium mt-0.5 tracking-tight">Manage and monitor your delivery personnel</p>
            </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-lg text-amber-700">
                        ...
                        <p className="text-sm font-medium">
                            Are you sure you want to deactivate <b>{boyToDeactivate?.firstName} {boyToDeactivate?.lastName}</b>?
                        </p>
                    </div>
                    <p className="text-xs text-slate-500">
                        Deactivating a delivery boy will prevent them from accepting new orders until they are reactivated.
                    </p>
