...
const ProductCard: React.FC<ProductCardProps> = ({ product, onClick, isWishlistPage }) => {
  ...
  const handleMoveToCart = useCallback(async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      // Only mark success & remove from wishlist if addToCart didn't throw
      setAddedToCart(true);
        ...
        <div className="flex items-center justify-between gap-1.5 mt-0">
          ...
          {isWishlistPage ? (
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#8B6914] px-3 py-2 text-[12px] font-semibold text-white hover:bg-[#7A5C10] transition leading-none whitespace-nowrap"
            >
            ...
            <div className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#D9C89A] bg-[#F8F1E1] px-2 py-1.5">
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-md bg-[#8B6914] text-lg font-bold text-white leading-none"
              >
                −
              </button>
              <span className="min-w-[1.4rem] text-center text-[14px] font-bold text-[#1A1A1A]">{cartQuantity}</span>
              <button
                type="button"
            ...
            <button
              ...
              className={`inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[12px] font-semibold transition-all leading-none whitespace-nowrap ${
